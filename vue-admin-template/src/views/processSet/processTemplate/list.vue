    ...
    >
      ...
      <el-table-column label="操作" width="250" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status == 0" type="text" size="mini" @click="edit(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.templateSet')  === false">修改审批设置</el-button>
          <el-button v-if="scope.row.status == 0" type="text" size="mini" @click="removeDataById(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.remove')  === false">删除</el-button>
...
export default {
  ...
  methods: {
    ...
    edit(id) {
      this.$router.push('/templateSet?id=' + id)
    }
  }
}
</script>
