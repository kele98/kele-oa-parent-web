    ...
    >
      ...
      <el-table-column label="操作" width="180" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="danger" icon="el-icon-delete" size="mini" title="删除" :disabled=" scope.row.children === null ? false : true  " @click="removeDataById(scope.row.id)" />
        </template>
      </el-table-column>
    </el-table>
...
<script>
import api from '@/api/admin/system/sysMenu'
const defaultForm = {
  id: '',
  parentId: '',
  name: '',
...
export default {
  ...
  methods: {
    ...
    // 根据id更新记录
    updateData() {
      api.update(this.sysMenu).then(response => {
        this.$message.success(response.message || '操作成功')
        this.dialogVisible = false
        this.fetchData()
      })
